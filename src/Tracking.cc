...
///构造函数   这个构造函数在system的构造函数中被调用，在system的构造函数中会生成一个tracking对象    
Tracking::Tracking(
    System *pSys,                       //系统实例
        ...
        mnLastRelocFrameId(0)                               //恢复为0,没有进行这个过程的时候的默认值
{
    ...
    DistCoef.copyTo(mDistCoef);   // distortion coefficient  畸变系数
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im,const double &timestamp)
{
    ...
    // Step 2 ：构造Frame，其中最重要的就是建立图像金字塔，提取FAST角点，并且计算对应的ORB描述子
    //判断该帧是不是初始化
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                //; mVelocity是cv::Mat类型，实际上就是上上帧到上一帧的坐标变换关系
                //; 这里不要理解为是为了刚初始化完毕之后跟踪而准备的，而要理解成重定位之后，
                //;   上一普通帧已经丢失了，所以这里只能在之前的关键帧中选择一个进行跟踪。
                if(mVelocity.empty() || mCurrentFrame.mnId < mnLastRelocFrameId+2)
                {
                    // 用最近的关键帧来跟踪当前的普通帧
                    // 通过BoW的方式在参考帧中找当前帧特征点的匹配点
                    // 优化每个特征点都对应3D点重投影误差即可得到位姿
                    //; 考虑 参考关键帧 到底是什么样的关键帧，是什么时候被创建的？
                    //; 如果暂时不考虑重定位的话，其实有两个创建的时刻：
                    //; 1.在TrackLocalMap里面，和当前帧具有最大的共视关系的那个关键帧就被赋值为了当前帧的
                    //;   RefKF
                    //; 2.在后面判断是否要生成新的关键帧，如果要生成，那么RefKF就变成新生成的这个关键帧

                    //; 所以说上面这两种方式都是有道理的:1的选择方式是考虑空间上的最大共视程度，2的选择方式
                    //; 是考虑时间上的最近一帧。比如下面使用恒速模型跟踪失败了，比如可能因为这一帧发生了一个
                    //; 快速的运动，超过了恒速模型半径搜索的范围假设，那么使用1的Ref是紧邻的上一帧的空间上
                    //; 共视程度最大的关键帧，肯定和当前帧的共视程度也比较大。如果使用2的Ref，那么只有一种
                    //; 情况，上一帧恰好生成了关键帧，然后当前帧发生快速运动导致恒速运动模型跟丢了，那么
                    //; 使用2的Ref就相当于把上一帧作为了参考关键帧，时间上他们是相邻的，所以也是合理的。
                    //; 然后到局部地图跟踪的时候，程序又会去寻找和当前帧共视程度最高的关键帧作为Ref，
                    //; 如果2的Ref不是共视程度最高的，那么它还是会被替换掉。
            ...
            //; 看这里的程序结构，也就是如果跟丢了，对来的每一帧都会尝试进行重定位，直到定位成功。所以这里就是在进行重定位
            else
            {
                // 如果跟踪状态不成功,那么就只能重定位了
                // BOW搜索，EPnP求解位姿
                bOK = Relocalization();   //; 重定位后，只会更新 上次重定位的Id为当前帧的Id
                //; 这里重定位会通过优化得到重定位后自己的一个初始位姿，同时得到当前帧的2D特征点和已有的3D地图点之间的一个匹配关系
            }
        }
        else        
        {
        ...
        // 将最新的关键帧作为当前帧的参考关键帧
        //; 下面这句其实用处不大了，因为马上就会被后面TrackLocalMap中更改RefKF
        mCurrentFrame.mpReferenceKF = mpReferenceKF;   // 更新变量了，把当前帧的参考关键帧 赋值为 Tracking的参考关键帧
        //; 如果从重定位的角度来看，上面这个赋值显然是没有任何意义的，因为重定位后当前帧在哪里都不知道。
        //; 所以下面接着进行TrackLocalMap，这里面会查看当前帧的地图，寻找和当前帧共视程度最大的那个关键帧作为RedKF。
        //; 这样到下一个循环的时候，使用TrackReferenceKeyFrame进行跟踪，此时的Ref就是有意义的了
        ...
        {
            if(bOK)
                //; 注意这个函数里面在跟踪局部地图之前会先更新局部地图，也就是首先更新局部关键帧，
                //; 然后就会更新当前帧和Tracking参考关键帧为当前帧的局部关键帧中共视程度最大的那个关键帧
                bOK = TrackLocalMap();
        }
        else
        {
        ...
        if(bOK)
        {
            ...
            // Clean VO matches
            // Step 6：清除观测不到的地图点   
            //; 也就是说这次Tracking马上完成了，但是只有关键帧观测到地图点才会增加地图点被观测的次数，如果当前帧匹配的地图点是没有被关键帧观测到的地图点，
            //; 那么就删除这个地图点和当前帧的匹配关系
            ...
            for(int i=0; i<mCurrentFrame.N; i++)
            {
                ...
                if(pMP)
                    //; 观测到这个地图点的相机数目<1，说明这个地图点没有被观测到过
            ...
            if(NeedNewKeyFrame())
                //; 注意下面这个函数里面，把当前帧创建成一个新的关键帧之后，会把Tracking和当前帧的参考关键帧都变成最新创建的这个参考关键帧
                CreateNewKeyFrame();
            ...
            for(int i=0; i<mCurrentFrame.N;i++)
            {
                ...
                //; 所以说这个在跟踪局部地图点的函数里面，BA优化之后产生了很多外点没有剔除，是统一在这里剔除了
                if(mCurrentFrame.mvpMapPoints[i] && mCurrentFrame.mvbOutlier[i])
                    mCurrentFrame.mvpMapPoints[i]=static_cast<MapPoint*>(NULL);
            }
        }
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mpInitializer)
    {
        ...
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            // 用当前帧更新上一帧
            mLastFrame = Frame(mCurrentFrame);
            // mvbPrevMatched  记录"上一帧"所有特征点的坐标pt
    ...
    else    //如果单目初始化器已经被创建
    {
        ...
        // Step 3 在mInitialFrame与mCurrentFrame中找匹配的特征点对
        ORBmatcher matcher(
            //; 对0.9这个取值存疑，初始化应该要求比较高啊？
            // 最佳的和次佳特征点评分的比值阈值，这里是比较宽松的，跟踪时一般是0.7
        ...
        int nmatches = matcher.SearchForInitialization(
            ...
            mvbPrevMatched,                 //在初始化参考帧中提取得到的特征点的坐标
            mvIniMatches,                   //保存匹配关系, 这里存储的是1中的特征点对应在2中的特征点的索引，如果是-1那么说明没有匹配
            100);                           //搜索窗口大小
            ...
            vbTriangulated))    //以及对应于mvIniMatches来讲,其中哪些点被三角化了
        {
            ...
            mCurrentFrame.SetPose(Tcw);  //; Tcw是第二帧，也就是当前帧的位姿

            // Step 8 创建初始化地图点MapPoints
            // Initialize函数会得到mvIniP3D，
            // mvIniP3D是cv::Point3f类型的一个容器，是个存放3D点的临时变量，只有坐标信息而没有其他任何信息，不满足地图点的特性
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // 注意mpMap是一个全局地图句柄，表明这个关键帧是属于某个全局地图的？
    //; mpKeyFrameDB是加载的离线词袋的字典，使用这个字典才能计算一张图片对应的词袋向量
    //; 注意下面，第一个关键帧的Id = 0, 因为里面赋值的时候是后置的++，也就是先赋值后+，所以前两帧的id分别是0和1
    ...
    {
        // 没有匹配，跳过
        if(mvIniMatches[i]<0)  //; 注意这里在三角化地图点之后，对于三角化不成功的，后来已经手动删除了特征点的匹配关系，所以这里只要是有匹配关系的，都是有对应的3D点的
            continue;
        ...
        //; 注意创建地图点的时候一些变量的默认值： nObs(0), mnVisible(1), mnFound(1)
        MapPoint* pMP = new MapPoint(
            worldPos,     // 地图点的世界坐标
            //; 看这里的设置，是把第二帧作为生成的初始地图点的参考关键帧
            pKFcur,       // 生成该地图点的关键帧，这个关键帧也作为这个地图点的参考关键帧
            mpMap);       // 地图点所在的地图
        ...
        //; 向关键帧中添加这个地图点，
        pKFini->AddMapPoint(pMP,i);  //; i是这个地图点对应于第一个关键帧中的特征点的索引
        pKFcur->AddMapPoint(pMP,mvIniMatches[i]);   //; mvIniMatches[i]是这个地图点对应于第二个关键帧中的特征点的索引

        // a.表示该MapPoint可以被哪个KeyFrame的哪个特征点观测到
        //; 注意这一步是建立公式关系的重要的一步
        //; 这里添加地图点的观测关系的时候，对每个地图点，它的nObs都会+1（对于单目来说是1，双目和RGBD是2）
        pMP->AddObservation(pKFini,i);   //; 添加地图点的观测，里面的数据类型是map，表明这个地图点可以被哪个关键帧观测到，对应在这个关键帧中的特征点索引是多少
        ...
        //; ！！ 注意这里和上面是不一样的，这里是给当前普通帧添加对地图点的观测，而上面是对建立的关键帧添加对地图点的观测
        mCurrentFrame.mvpMapPoints[mvIniMatches[i]] = pMP;  // 设置当前帧（注意不是关键帧）中特征点对应的地图点
        mCurrentFrame.mvbOutlier[mvIniMatches[i]] = false;  // 当前帧的特征点有匹配关系，那么他就不是外点
    ...
    // 在关键帧和关键帧之间建立边，每个边有一个权重，边的权重是该关键帧与其他所有的关键帧观测到的共同的地图点的个数
    ...
    //; 注意看传入的变量，是地图，也就是说优化的时候直接优化地图就可以，因为地图中就包括关键帧和地图点，这就包括了BA优化中的所有变量
    //; 进行GBA的时候固定第0帧的位姿，优化其他位姿
    Optimizer::GlobalBundleAdjustemnt(mpMap,20);

    // Set median depth to 1
    // Step 5 取场景的中值深度，用于尺度归一化 
    // 为什么是 pKFini 而不是 pKCur ? 答：都可以的，内部做了位姿变换了
    //; CC：还是不太一样把，因为这两帧看到的地图点是不一样的
    //; CC: 有考虑了一下是一样的，因为现在是创建初始地图，这个初始地图就是由初始帧和当前帧三角化得到的，所以这两帧对应的地图是一样的
    ...
    //; 为什么要进行这样的尺度归一化呢？或者说原来算出来的t是在什么尺度下的？
    //; CC:简单理解，是先通过对极几何计算出了R和t，然后通过两帧之间的Rt得到投影矩阵，然后三角化得到地图点，所以说地图点可以看成是和Rt一个单位的
    //; 这里地图点和t都除以尺度中值后，他们还是等价的
    // Scale initial baseline
    cv::Mat Tc2w = pKFcur->GetPose();
    // x/z y/z 将z归一化到1 
    //; 注意这里只是归一化了平移向量t，因为只有平移是有尺度的，而旋转是绝对的
    ...
    {
        if(vpAllMapPoints[iMP])    // 如果当前帧的这个地图点不是空，说明这个特征点的位置有对应的地图点
        {
            MapPoint* pMP = vpAllMapPoints[iMP];
            pMP->SetWorldPos(pMP->GetWorldPos()*invMedianDepth);     // 那这里只是把初始帧对应的地图点进行了归一化？
            //; 其实是没有问题的，因为根据前面单目生成地图点可以知道，地图点是初始帧和当前帧一起三角化算出来的，所以这两帧对应的地图点是完全一样的
        }
    }

    //; 注意Step 6 和 7进行尺度归一化，把平移和地图都乘了同一个尺度因子，这样是可行的，只不过这个尺度因子选的是初始地图的深度中值
    ...
    //; 注意这里！这里是处理当前帧，这是个普通帧
    mCurrentFrame.SetPose(pKFcur->GetPose());

    //; 再注意这里！这里是直接更新为下一次做准备了
    //; 这里写法是不是不太严谨？索然结果都是1，但是这里明显是想把当前关键帧的ID赋值给左侧的，但是这里是使用了当前帧（普通帧）的id
    mnLastKeyFrameId=mCurrentFrame.mnId;        
    mpLastKeyFrame = pKFcur;

    //; ！！注意这里的局部关键帧，在哪里使用了？？？
    //; CC: 在跟踪线程的TrackLocalMap的时候就使用了！
    mvpLocalKeyFrames.push_back(pKFcur);
    mvpLocalKeyFrames.push_back(pKFini);
    // 单目初始化之后，得到的初始地图中的所有点都是局部地图点
    mvpLocalMapPoints=mpMap->GetAllMapPoints();

    //; 注意这里的RefKF是Tracking的
    ...
    //; ！！设置参考地图点用于绘图显示局部地图点（红色），这里还是要看一下
    mpMap->SetReferenceMapPoints(mvpLocalMapPoints);

    mpMapDrawer->SetCurrentCameraPose(pKFcur->GetPose());

    //; ！!保存初始关键帧，为什么要用一个set来存，不是只有一个吗？
    mpMap->mvpKeyFrameOrigins.push_back(pKFini);
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    // Step 2：通过词袋BoW加速当前帧与参考帧之间的特征点匹配
    //; 思考这里为什么要使用词袋匹配？
    //; CC：考虑重定位之后跟踪的情况，因为这个时候上一帧已经跟丢了，没有运动模型可以使用，所以特征匹配就不能利用一帧的地图点投影当当前帧的方法，
    //; 所以只能使用词袋跟踪。
    int nmatches = matcher.SearchByBoW(
        //; 这个函数在初始化之后第一次跟踪的时候调用的话，那么参考关键帧就是初始化的时候用的第2帧，作为Tracking的参考关键帧。
        //; !! 那么重定位之后呢？Tracking的参考关键帧变成了什么？
    ...
    //; ！！ 这里存疑啊？？既然都已经跟丢了，这里还用上一帧的位姿，我感觉意义不大。而且既然上面已经匹配了参考关键帧和当前帧的匹配关系，
    //; 为什么不先算个PnP作为优化的初值呢？
    ...
    int nmatchesMap = 0;
    for(int i =0; i<mCurrentFrame.N; i++)  //; N是2D特征点
    {
        if(mCurrentFrame.mvpMapPoints[i])  // 有地图点
        {
            ...
            if(mCurrentFrame.mvbOutlier[i])
            {
                ...
                // 当前帧对应的地图点匹配关系删除
                mCurrentFrame.mvpMapPoints[i]=static_cast<MapPoint*>(NULL);
                // 没有地图点了，指示地图点是否是外点自然也没有意义了
                //; 注意上面的解释是不对的，地图点是一直存在的，只是删除了当前帧和地图点的匹配关系
                mCurrentFrame.mvbOutlier[i]=false;  
                //; mbTrackInView变量是在后面TrackLocalMap的时候判断是否要把局部地图中的点投影到当前帧中进行进一步的特征匹配的，
                //; 是true的话就进行投影。这里使用参考关键帧跟踪优化后发现这个点是外点，那么肯定后面也不要投影它了
                pMP->mbTrackInView = false; 
                pMP->mnLastFrameSeen = mCurrentFrame.mnId;  //; ???
                nmatches--;
            }
            else if(mCurrentFrame.mvpMapPoints[i]->Observations()>0)   //; 为什么还要判断这个条件呢？
                //匹配的内点计数++
                nmatchesMap++;
        }
    }
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // Step 1：更新上一帧的位姿；对于双目或RGB-D相机，还会根据深度值生成临时地图点
    //; 为什么要更新上一帧的位姿？
    //; 因为关键帧在LocalMapping线程中会被优化，位姿会更新，而上一帧和它的参考关键帧相邻比较近，
    //; 所以他们之间的位姿相对来说还是比较准确的，所以这里就使用他们之间的位姿关系 * 参考关键帧优化后的位姿，
    //; 得到上一帧“间接”优化后的位姿
    UpdateLastFrame(); 
    ...
    //; 经过这个函数之后，mCurrentFrame:mvpMapPoints就不再全是空了，里面的部分2D特征点就有匹配的3D地图点了
    ...
    if(mbOnlyTracking)
    {
        // 纯定位模式d：如果成功追踪的地图点非常少,那么这里的mbVO标志就会置位
        mbVO = nmatchesMap<10;
        return nmatches>20;
    }
...
bool Tracking::TrackLocalMap()
{
    ...
    //; 注意这个函数里面会更新当前帧和Tracking参考关键帧为 当前帧的共视关键帧中共视程度最大的那个关键帧
    UpdateLocalMap();

    // Step 2：筛选局部地图中新增的在视野范围内的地图点，投影到当前帧搜索匹配，得到更多的匹配关系
    //; 这个函数内部最后会调用SearchByProjection()，把当前帧的2D特征点中还没有匹配上地图点的那些，再次和Tracking的局部地图点进行匹配
    SearchLocalPoints();
    ...
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            ...
            {
                // 找到该点的帧数mnFound 加 1
                //; 这个地图点被发现的次数+1，注意这里每一帧只要观测到了这个地图点都会增加这个被找到的次数，并不是只有关键帧才可以！
                //; 这里是found变量，就是成功匹配上的次数
                mCurrentFrame.mvpMapPoints[i]->IncreaseFound();
                //查看当前是否是在纯定位过程
                if(!mbOnlyTracking)
                {
...
void Tracking::CreateNewKeyFrame()
{
    ...
    //; 注意下面两个，Tracking的RefKF和当前帧的RefKF是同步更新的，都是同一个关键帧
    mpReferenceKF = pKF;
    ...
    // 当前帧成为新的关键帧，更新。
    //; 注意这里已经到了一次Tracking的最后，所以这里更新是为了下一次的Tracking做准备
    //; 看这里是有明显错误的？如果没有错的话，只能理解成mnLastKeyFrameId是上一个关键帧对应的普通帧的Id(因为关键帧最基础的来说肯定是一个普通帧)
    mnLastKeyFrameId = mCurrentFrame.mnId;   
    mpLastKeyFrame = pKF;
}
    ...
    for(vector<MapPoint*>::iterator vit=mCurrentFrame.mvpMapPoints.begin(), vend=mCurrentFrame.mvpMapPoints.end(); vit!=vend; vit++)
    {
        ...
        if(pMP)
        {
            ...
            else
            {
                ...
                pMP->IncreaseVisible();
                // 标记该点被当前帧观测到
                //; 初步猜测这个mnLastFrameSeen是和上面的Visible配合使用的，也就是看到了这个点，即这个点在我的视野范围内，
                //; 但是这个地图点不一定匹配上了当前帧的2D特征点
                pMP->mnLastFrameSeen = mCurrentFrame.mnId;
    ...
    while(nCandidates>0 && !bMatch)
    {
        ...
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(!Tcw.empty())
            {
                ...
                //; 注意这里优化之后会把当前帧的位姿直接更新
                int nGood = Optimizer::PoseOptimization(&mCurrentFrame);

                // 如果优化之后的内点数目不多，跳过了当前候选关键帧,继续使用下一个候选关键帧进行重定位
                if(nGood<10)
                    continue;
