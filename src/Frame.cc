 ...
 */
          ...
          mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
    {
        ...
        ExtractORB(0, imGray); // 此函数进行了封装，内部是调用了ORBextractor的()仿函数
 ...
 */
    void Frame::AssignFeaturesToGrid()
    {
        ...
        int nReserve = 0.5f * N / (FRAME_GRID_COLS * FRAME_GRID_ROWS); // N是特征点个数
    ...
    /**
 * @brief 判断地图点是否在视野中  Frustum圆锥体，可以认为就是视角
 * 步骤
 ...
 */
    ...
    {
        // Step 1 如果第一个畸变参数为0，不需要矫正。第一个畸变参数k1是最重要的且一般不为0，为0的话说明畸变参数都是0
